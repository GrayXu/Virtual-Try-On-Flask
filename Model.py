...
class Model(object):
    ...
    def getPoseData(self, pose):
        '''
        输入jppnet的pose结果 with shape (?,?,3)
        输出：
            CPVTON所需要的pose
            和根据pose估算得到的置信度
    ...
    def getPoseMap(self, pose_data):
        '''
        传入的pose_data也为np array, shape为(16,3)
        返回：
            对所有的pose位置，绘制一个大小为3*3的白色正方形，作为人体特征的一部分
        '''
    ...
    def predict(self, human_img, c_img, need_pre=True, need_bright=False, keep_back=False, need_dilate=False, check_dirty=False):
        ...
        pose_data,trusts = self.getPoseData(pose)
        
        if need_pre:
            human_img,pose_data,parse = self.cropByPoseData(human_img, pose_data,parse)
        
        if pose_data is None: # no person and pose data error
            return human_img, 0.0
        if check_dirty and self.is_dirty(pose_data[10],pose_data[15],pose_data[12],pose_data[2],pose_data[3],pose_data[13]): # drity data(e.g. cross hand)?
            return human_img, 0.0       
        
        
        pose_map = self.getPoseMap(pose_data)
        
        # fix: 把左右腿的识别全部当做裤子（迎合标注）
    ...
    def cropByPoseData(self, img, pose_data, parse):
        '''
        ...
        height = max([pose_data[2][1],pose_data[3][1],pose_data[10][1],pose_data[15][1]]) - pose_data[9][1]
        #上下的裁剪位置
        pre_height = max([pose_data[2][1],pose_data[3][1],pose_data[10][1],pose_data[15][1]])-pose_data[9][1]
        upper = max(pose_data[9][1]-int(pre_height*0.2) , 0)
        bounder = min(max([pose_data[2][1],pose_data[3][1],pose_data[10][1],pose_data[15][1]])+int(pre_height*0.2), h)
        ...
        left = int(left)
        right = int(right)
        print("upper:%d,bounder:%d,left:%d,right:%d"%(upper,bounder,left,right))
    ...
    def is_dirty(self,wrist_a, wrist_b,a,b,c,d):
        '''
        ...
        Note: For JPP Net's output pose, the order is [10,15,12,2,3,13]
        '''
        margin = 250 # 手腕部接近四个点则不算dirty 
        KB_list = [self.get_K_b(a,b),self.get_K_b(b,c),self.get_K_b(c,d),self.get_K_b(a,d)]
        ...
        if self.right_line(wrist_a,KB_list[0],wrist_a[0]) and self.upon_line(wrist_a,KB_list[1]): 
            print("wrist_a dirty")
            return True
        if not self.right_line(wrist_b,KB_list[2],wrist_b[0]) and self.upon_line(wrist_b,KB_list[1]):
            print("wrist_b dirty")
            return True
        else:
            return False